...
class FrankaLiberoEnv(BaseEnv):
    ...
    def __init__(
        ...
        self.gripper_link_idx = self._body_name2id_any("gripper0_eef", "gripper0_right_eef")
    ...
    def _body_name2id_any(self, *names: str) -> int:
        for name in names:
            try:
                return self.handle.env.sim.model.body_name2id(name)
            except ValueError:
                continue
        raise ValueError(f"None of the body names exist: {names}")
