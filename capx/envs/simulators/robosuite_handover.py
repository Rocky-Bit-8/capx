...
from capx.utils.robot_description import load_local_panda_description
...
class RobosuiteHandoverEnv(BaseEnv):
    def __init__(
        ...
        # Keep these flags on the low-level environment.  CodeExecutionEnvBase
        # uses them after each executed program to refresh the Viser scene.
        ...
        self._video_subsample_rate = 1  # Record every MuJoCo frame.
        ...
        if viser_debug:
            ...
            # Non-robot MuJoCo visual geoms are mirrored into Viser.  Robot
            # geometry remains the existing articulated URDF visualization.
            self.mujoco_geom_handles: dict[int, Any] = {}
            self._init_mujoco_scene()
            self.gripper_metric_length = 0.0584
            
            # Load URDF for both robots
            self.urdf = load_local_panda_description()
    ...
    def move_to_joints_blocking(
        ...
        while steps < max_steps:
            ...
            # Do not query / render the simulator from inside a MuJoCo control
            # step.  Viser updates call ``get_observation()``, which performs a
            # camera render and ``sim.forward()``.  With two-arm robosuite this
            # can re-enter MuJoCo while ``step()`` is still unwinding and cause
            # a native SIGFPE.  CodeExecEnvBase refreshes Viser once after the
            # complete user code block instead.
    ...
    def move_to_joints_blocking_arm1(
        ...
        while steps < max_steps:
            ...
            # See move_to_joints_blocking: defer Viser refresh until the code
            # block has completed, outside the MuJoCo stepping loop.
    ...
    def _step_once(self) -> None:
        ...
        # Viser is refreshed after the complete program, not during this
        # MuJoCo step; see move_to_joints_blocking.
    ...
    def _init_mujoco_scene(self) -> None:
        """Create Viser nodes for visible, non-robot MuJoCo primitive geoms."""
        ...
        for geom_id in range(model.ngeom):
            ...
            # Panda links are already represented by ViserUrdf.  Robosuite uses
            # mesh geoms for them, while this task's arena and hammer are
            # primitives; ignoring meshes avoids duplicate robot geometry and a
            # multi-hundred-thousand-vertex transfer on every web session.
            ...
            if rgba[3] <= 0.0:
                continue
            color = tuple(np.clip(np.rint(rgba[:3] * 255), 0, 255).astype(int))
            name = model.geom_id2name(geom_id) or f"geom_{geom_id}"
            path = f"/mujoco/{name}_{geom_id}"
            size = np.asarray(model.geom_size[geom_id], dtype=np.float64)

            if geom_type == 0:  # mjGEOM_PLANE
                handle = self.viser_server.scene.add_box(
                    path, color=color, dimensions=(2 * size[0], 2 * size[1], 0.01),
                    opacity=float(rgba[3]), receive_shadow=True,
                )
            elif geom_type == 2:  # mjGEOM_SPHERE
                handle = self.viser_server.scene.add_icosphere(
                    path, color=color, radius=float(size[0]), opacity=float(rgba[3]),
                )
            elif geom_type == 5:  # mjGEOM_CYLINDER
                handle = self.viser_server.scene.add_cylinder(
                    path, color=color, radius=float(size[0]), height=float(2 * size[1]),
                    opacity=float(rgba[3]),
                )
            elif geom_type == 6:  # mjGEOM_BOX
                handle = self.viser_server.scene.add_box(
                    path, color=color, dimensions=2 * size, opacity=float(rgba[3]),
                )
            else:
                continue
            self.mujoco_geom_handles[geom_id] = handle

        self._update_mujoco_scene()

    def _update_mujoco_scene(self) -> None:
        """Synchronize mirrored MuJoCo geom transforms in robot0-base coordinates."""
        if not self.mujoco_geom_handles:
            return

        data = self.robosuite_env.sim.data
        base0_world = vtf.SE3(wxyz_xyz=self.base_link_wxyz_xyz_0)
        world_to_base0 = base0_world.inverse()
        for geom_id, handle in self.mujoco_geom_handles.items():
            rotation_wxyz = SciRotation.from_matrix(
                np.asarray(data.geom_xmat[geom_id], dtype=np.float64).reshape(3, 3)
            ).as_quat()
            geom_world = vtf.SE3.from_rotation_and_translation(
                rotation=vtf.SO3(wxyz=np.array([
                    rotation_wxyz[3], rotation_wxyz[0], rotation_wxyz[1], rotation_wxyz[2]
                ])),
                translation=np.asarray(data.geom_xpos[geom_id], dtype=np.float64),
            )
            geom_base0 = world_to_base0 @ geom_world
            handle.position = geom_base0.translation()
            handle.wxyz = geom_base0.rotation().wxyz
