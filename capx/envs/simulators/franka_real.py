    ...
    def move_to_joints_blocking(
        ...
        while steps < max_steps:
            ...
            if joints_obs is not None:
                joints_arr = np.asarray(joints_obs, dtype=np.float64).reshape(-1)
                # Real Franka (no gripper) reports 7 arm joints; the legacy sim
                # format is 8 (7 arm joints + gripper as the last element).
                if joints_arr.shape[0] == 8:
                    joints_arr = joints_arr[:-1]
                current = joints_arr.reshape(7)
