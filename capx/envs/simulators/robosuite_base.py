...
class RobosuiteBaseEnv(BaseEnv):
    ...
    _VIDEO_SUBSAMPLE_RATE: int = 1  # Record every MuJoCo frame.
    _ACTION_SLICE: int = -1  # action[:-1] for most envs, action[:-2] for spill_wipe
    # Viser is a debugger, so it does not need a camera/point-cloud refresh at
    # the control-step rate. Subclasses can override this for interactive use.
    _VISER_UPDATE_RATE: int = 20
    ...
    def _do_robosuite_step(self, action: np.ndarray) -> bool:
        """Step robosuite with the given action, handling render skipping.

        A generated code block can contain further motions after a task has
        completed. Robosuite rejects those actions; treat them as no-ops so a
        successful episode remains successful instead of failing mid-block.
        """
        if getattr(self.robosuite_env, "done", False):
            return False
        sliced = action[:self._ACTION_SLICE] if self._ACTION_SLICE != 0 else action
        # Viser refreshes call get_observation() and render camera data
        # separately. Do not force an expensive camera render on every physics
        # step merely because the debug server exists.
        need_render = self._record_frames and self._sim_step_count % self._VIDEO_SUBSAMPLE_RATE == 0
        if need_render:
    ...
    def _step_once(self) -> None:
        ...
        if not self._do_robosuite_step(action):
            return
        ...
        if (
            hasattr(self, "viser_server")
            and self._sim_step_count % self._VISER_UPDATE_RATE == 0
        ):
            ...
            for image_key in rbg_imgs:
                ...
                if "depth" in obs[image_key].get("images", {}):
                    points, colors = depth_color_to_pointcloud(
                        ...
                        # The point cloud is for debugging only; keep the
                        # rendered camera observation at its native resolution.
                        subsample_factor=4,
                    )
