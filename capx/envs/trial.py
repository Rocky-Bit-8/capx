...
def _save_trial_video(
    ...
    *,
    ...
    video_fps = int(getattr(getattr(env, "low_level_env", env), "_VIDEO_FPS", 30))
    suffix = f"{reward:.3f}"
    if suffix_extra:
        suffix += f"_{suffix_extra}"

    if isinstance(frames, list):
        _write_video(frames, base_dir, suffix=suffix, fps=video_fps)
    elif isinstance(frames, dict):
        for key, frame in frames.items():
            _write_video(frame, base_dir, suffix=f"{suffix}_{key}", fps=video_fps)
...
def _save_turn_and_combined_videos(
    ...
    if isinstance(all_frames, dict):
        ...
        for key, frames in all_frames.items():
            if frames:
                _write_video(frames, base_dir, suffix=f"combined_{key}", fps=video_fps)
        return
    ...
    for i, (start, end) in enumerate(turn_frame_ranges):
        ...
        if turn_frames:
            _write_video(turn_frames, base_dir, suffix=f"turn_{i:02d}", fps=video_fps)

    # Combined video
    _write_video(all_frames, base_dir, suffix="combined", fps=video_fps)
    ...
    if config.get("use_wrist_camera") and hasattr(env, "get_wrist_video_frames"):
        ...
        if wrist_frames:
            for i, (start, end) in enumerate(turn_frame_ranges):
                ...
                if wrist_turn:
                    _write_video(wrist_turn, base_dir, suffix=f"turn_{i:02d}_wrist", fps=video_fps)
            _write_video(wrist_frames, base_dir, suffix="combined_wrist", fps=video_fps)
...
def _handle_multi_turn_step(
    ...
    complete_multi_turn_prompt += (
        "\n\nEnvironment evaluation after the executed code:\n"
        f"- reward: {info_step.get('reward', 0.0)}\n"
        f"- task_completed: {info_step.get('task_completed', False)}\n"
        f"- sandbox_rc: {info_step.get('sandbox_rc', -1)}\n"
        "If task_completed is False, do not output FINISH. Output REGENERATE "
        "with a different strategy than the previously executed code.\n"
    )
    ...
    if decision == "finish" and not info_step.get("task_completed", False):
        repair_prompt = copy.deepcopy(decision_prompt)
        repair_prompt[-1]["content"].append({
            "type": "text",
            "text": (
                "The environment explicitly reports task_completed=False. "
                "Your previous response chose FINISH, which is invalid. "
                "Respond with REGENERATE and provide a substantially different "
                "Python code block. Avoid repeating the same grasp/place sequence."
            ),
        })
        repair_prompt[-1]["content"] = collapse_text_image_inputs(repair_prompt[-1]["content"])
        content = _query_model(args, repair_prompt)
        reasoning = content["reasoning"]
        decision, new_code = _parse_multi_turn_decision(content["content"])
...
def _run_single_trial(
    ...
    while code_block_idx < len(code_blocks) and code_block_idx <= MULTITURN_LIMIT:
        ...
        info_step["task_completed"] = info_step.get("task_completed", env.task_completed())
        ...
        if multi_turn_prompt:
            ...
            if decision == "regenerate":
                ...
                # Continue from the current physical state so the next turn
                # can repair or complete the actions already performed.
