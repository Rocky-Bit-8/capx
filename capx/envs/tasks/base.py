...
def _capture_motion_diagnostics(env: BaseEnv) -> dict[str, np.ndarray] | None:
    sim = None
    ...
    joints = None
    joint_addrs = getattr(env, "_panda_joint_qpos_addrs", None)
    try:
        if joint_addrs is not None:
            joints = np.array(sim.data.qpos[joint_addrs], dtype=np.float64)
        else:
            joints = np.array(sim.data.qpos[:7], dtype=np.float64)
    except Exception:
        joints = None
    ...
    if body_id is not None:
        try:
            ee_pos = np.array(sim.data.xpos[body_id], dtype=np.float64)
            ee_quat = np.array(sim.data.xquat[body_id], dtype=np.float64)
        except Exception:
            ee_pos = None
            ee_quat = None

    if joints is None and ee_pos is None:
        return None
...
def _format_motion_diagnostics(before: dict[str, np.ndarray] | None, after: dict[str, np.ndarray] | None) -> str:
    if before is None or after is None:
        return ""
    lines = ["\n[Motion diagnostics]"]
    if "joints" in before and "joints" in after:
        joint_delta = after["joints"] - before["joints"]
        lines.append(
            "joint_delta_norm={:.6f}, joint_delta={}".format(
                float(np.linalg.norm(joint_delta)),
                np.array2string(joint_delta, precision=4, suppress_small=True),
            )
        )
    if "ee_pos" in before and "ee_pos" in after:
        ee_delta = after["ee_pos"] - before["ee_pos"]
        lines.append(
            "ee_delta_norm_m={:.6f}, ee_before={}, ee_after={}".format(
                float(np.linalg.norm(ee_delta)),
                np.array2string(before["ee_pos"], precision=4, suppress_small=True),
                np.array2string(after["ee_pos"], precision=4, suppress_small=True),
            )
        )
    return "\n".join(lines) + "\n"
...
class SimpleExecutor:
    ...
    def run(self, code: str, *, inputs: dict[str, Any] | None = None) -> dict[str, Any]:
        g: dict[str, Any] = {
            "__name__": "__main__",
            # Cube Stack code may use the documented numpy alias without an
            # explicit import in every generated block.
            "np": np,
            "env": self._env,
            "APIS": self._apis,
            "INPUTS": inputs or {},
...
class CodeExecutionEnvBase(Env):
    ...
    def task_completed(self) -> bool:
        """Proxy task completion from the low-level environment when available."""
        if hasattr(self.low_level_env, "task_completed"):
            return bool(self.low_level_env.task_completed())
        return bool(self.compute_reward() == 1.0)
    ...
    def _init_exec_globals(self) -> None:
        ...
        g: dict[str, Any] = {
            "__name__": "__main__",
            # Keep the numpy alias available across persistent code blocks.
            "np": np,
    ...
    def step(self, action: str) -> tuple[ObsType, SupportsFloat, bool, bool, dict[str, Any]]:
        ...
        """
        self._step_count += 1
        motion_before = _capture_motion_diagnostics(self.low_level_env)
        exec_result = self._exec_user_code(action)
        motion_after = _capture_motion_diagnostics(self.low_level_env)
        motion_diag = _format_motion_diagnostics(motion_before, motion_after)
        if motion_diag:
            exec_result["stdout"] += motion_diag
        obs = self._get_observation()
    ...
    def capture_video_frame(self) -> None:
        """Capture the current MuJoCo frame, including the final state."""
        record_frame = getattr(self.low_level_env, "_record_frame", None)
