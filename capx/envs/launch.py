...
from capx.llm.client import HIGHLAND_MODEL, HIGHLAND_SERVER_URL
    ...
    if not node_bin.exists():
        try:
            subprocess.check_call(
                [sys.executable, "-c", "import nodeenv"],
                stdout=subprocess.DEVNULL,
                stderr=subprocess.DEVNULL,
            )
        except subprocess.CalledProcessError:
            subprocess.check_call([sys.executable, "-m", "pip", "install", "-q", "nodeenv"])
        nodeenv_cmd = [sys.executable, "-m", "nodeenv", "--prebuilt", "--node=20.18.1"]
