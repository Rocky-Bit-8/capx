...
from capx.llm.client import HIGHLAND_MODEL, HIGHLAND_SERVER_URL, _api_key_for_model
...
# ---------------------------------------------------------------------------
# Viser chooses the next available port when its default (8080) is occupied.
# Web UI sessions can therefore land on 8082+ after a previous trial or a
# separate Viser process. Keep the scan bounded to Viser's default range.
_VISER_PORTS = range(8080, 8100)
_viser_port_cache: int | None = None


def set_viser_port(port: int | None) -> None:
    """Pin the proxy to the Viser instance created for the active session."""
    global _viser_port_cache
    _viser_port_cache = int(port) if port is not None else None
...
def create_app() -> FastAPI:
    ...
    async def load_config(request: LoadConfigRequest):
        ...
        try:
            ...
            class MinimalArgs:
                config_path: str
                server_url: str = HIGHLAND_SERVER_URL
                model: str = HIGHLAND_MODEL
                ...
                visual_differencing_model: str | None = HIGHLAND_MODEL
                visual_differencing_model_server_url: str | None = HIGHLAND_SERVER_URL
    ...
    async def start_trial(request: StartTrialRequest):
        ...
        try:
            ...
            load_args = LoadArgs(
                ...
                # Older Web UI clients do not send these optional overrides.
                # Leave them unset so _load_config uses the YAML values.
                record_video=getattr(request, "record_video", None),
                use_oracle_code=getattr(request, "use_oracle_code", None),
                use_parallel_ensemble=getattr(request, "use_parallel_ensemble", None),
                use_video_differencing=getattr(request, "use_video_differencing", None),
                use_wrist_camera=getattr(request, "use_wrist_camera", None),
                use_multimodel=getattr(request, "use_multimodel", None),
            )
            ...
            trial_args = LaunchArgsCompat(
                model=request.model,
                server_url=request.server_url,
                api_key=_api_key_for_model(request.model, None),
                max_tokens=request.max_tokens,
                temperature=request.temperature,
                ...
                visual_differencing_model=request.visual_differencing_model,
                visual_differencing_model_server_url=request.visual_differencing_model_server_url,
                visual_differencing_model_api_key=_api_key_for_model(
                    request.visual_differencing_model or request.model,
                    None,
                ),
            )
