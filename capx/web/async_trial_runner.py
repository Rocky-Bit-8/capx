...
MULTITURN_LIMIT = 30
# The initial VDM request is optional context for code generation. Do not let a
# stalled external vision API prevent an interactive trial from starting.
INITIAL_ENV_DESCRIPTION_TIMEOUT_SECONDS = 25
INITIAL_ENV_DESCRIPTION_MAX_TOKENS = 512
...
async def run_trial_async(
    ...
    try:
        ...
        # The task config can either name a registered low-level environment or
        # embed its constructor under ``low_level``.  The latter does not inherit
        # fields from CodeExecEnvConfig, so enable the nested renderer explicitly.
        if "cfg" in session.env_factory:
            session.env_factory["cfg"]["enable_render"] = True
            session.env_factory["cfg"]["viser_debug"] = True
            low_level_cfg = session.env_factory["cfg"].get("low_level")
            if isinstance(low_level_cfg, dict):
                low_level_cfg["enable_render"] = True
                low_level_cfg["viser_debug"] = True
        ...
        # Viser may choose a port other than 8080 when another instance is
        # still alive. Pin the reverse proxy to this session's actual server;
        # scanning for the first open port can attach the UI to a stale trial.
        viser_server = getattr(getattr(env, "low_level_env", env), "viser_server", None)
        if viser_server is not None and hasattr(viser_server, "get_port"):
            from capx.web.server import set_viser_port

            viser_port = int(viser_server.get_port())
            session.viser_port = viser_port
            set_viser_port(viser_port)
            logger.info("Pinned Viser proxy to port %d for session %s", viser_port, session.session_id)
        else:
            logger.error(
                "Web UI requested 3D visualization, but %s did not create a Viser server",
                type(getattr(env, "low_level_env", env)).__name__,
            )
        ...
        visual_differencing_args = ModelQueryArgs(
            ...
            # Environment descriptions are short context, not code generation.
            max_tokens=min(args.max_tokens, INITIAL_ENV_DESCRIPTION_MAX_TOKENS),
        ...
        use_oracle_code = session.config.get("use_oracle_code", False)
        ...
        if not use_oracle_code and use_img_differencing and initial_visual_feedback_base64:
            ...
            try:
                initial_env_description_out = await asyncio.wait_for(
                    asyncio.to_thread(
                        _query_model,
                        visual_differencing_args,
                        initial_env_description_prompt,
                    ),
                    timeout=INITIAL_ENV_DESCRIPTION_TIMEOUT_SECONDS,
                )
                initial_env_description = initial_env_description_out["content"]
                initial_visual_differencing_feedback = (
                    "The initial state of the environment is described as follows:\n"
                    f"{initial_env_description}"
                )
                obs["full_prompt"][-1]["content"][0]["text"] += (
                    f"\n\n{initial_visual_differencing_feedback}"
                )
                logger.info(
                    "Initial environment description completed in %.1fs for session %s",
                ...
                logger.warning(
                    "Initial environment description timed out after %ss for session %s; "
                    "continuing without VDM context",
                    INITIAL_ENV_DESCRIPTION_TIMEOUT_SECONDS,
                    session.session_id,
                )
                await emit(EnvironmentInitEvent(
                    session_id=session.session_id,
                    status="description_complete",
                    message="Initial image analysis timed out; continuing without it",
                ))
            except Exception:
                logger.exception(
                    "Initial environment description failed for session %s; "
                    "continuing without VDM context",
                    session.session_id,
                )
                await emit(EnvironmentInitEvent(
                    session_id=session.session_id,
                    status="description_complete",
                    message="Initial image analysis failed; continuing without it",
                ))
        ...
        # ========================================================================
        # Initial code generation (or built-in oracle execution)
        # ========================================================================
        ...
        await emit(ModelStreamingStartEvent(
            ...
            model_name="built-in oracle" if use_oracle_code else args.model,
        ))
        ...
        if use_oracle_code:
            ...
            if not raw_code:
                raise ValueError(f"{type(env).__name__} does not provide oracle_code")
            logger.info("Using built-in oracle code; skipping all model queries")
            await emit(ModelStreamingDeltaEvent(
                session_id=session.session_id,
                content_delta=raw_code,
            ))
        else:
        ...
        while code_block_idx < len(code_blocks) and code_block_idx <= MULTITURN_LIMIT:
            ...
            try:
                ...
                try:
                    ...
                    # Preserve the current state for the multi-turn decision.
            ...
            # Multi-turn decision
            # ====================================================================
            if multi_turn_prompt and not use_oracle_code:
                ...
                if decision == "regenerate":
                    ...
                    # Continue from the current state so the next turn can
                    # repair or complete actions already performed.
        ...
        if output_dir:
            ...
            if session.config.get("record_video") and hasattr(env, "get_video_frames"):
                # The final action may occur between periodic video samples.
                # Capture it on the MuJoCo environment thread before reading
                # the buffer so the exported video always includes final state.
                if hasattr(env, "capture_video_frame"):
                    await run_in_env_thread(env.capture_video_frame)
                ...
                if frames:
                    ...
                    await asyncio.to_thread(
                        ...
                        video_dir,
                        suffix=f"{reward:.3f}",
                        # Frames are captured every MuJoCo step. Encode at one third
                        # speed so the Viser/Web recording is inspectable on playback.
                        fps=20 / 3,
                    )
        else:
