...
HIGHLAND_MODEL = "kimi-k2.5"
HIGHLAND_SERVER_URL = "https://www.highland-api.top/v1/chat/completions"
# Qwen is served through Alibaba Cloud's OpenAI-compatible endpoint.
DASHSCOPE_MODEL = "qwen3.8-max"
DASHSCOPE_SERVER_URL = "https://token-plan.cn-beijing.maas.aliyuncs.com/compatible-mode/v1/chat/completions"
DASHSCOPE_API_KEY = "sk-sp-H.DRLDLY.kdi2.MEYCIQCohDr4NZ1D4qWVZNJnVfdz_rG5GimomUXQOTAYYweZ_AIhAOkuqrdjN61QK_L0AtJkm4iUn1Fd3zI2N_kVuos0QS3E"
DASHSCOPE_MAX_TOKENS = 4096
# Robot-control requests should fail fast enough for the UI to recover.
LLM_REQUEST_TIMEOUT = (30, 180)  # connect/write timeout, read timeout (seconds)
LLM_MAX_RETRIES = 2
...
ENSEMBLE_CONFIGS = [
    # Default ensemble candidates use the default Highland model.
    (HIGHLAND_MODEL, [0.1, 0.5, 0.9]),
]
...
def _api_key_for_model(model: str, explicit_api_key: str | None) -> str | None:
    if explicit_api_key:
        return explicit_api_key
    if model == DASHSCOPE_MODEL:
        return DASHSCOPE_API_KEY
    _load_local_env()
    if model in GPT_MODELS:
        return os.getenv("OPENAI_API_KEY")
    return os.getenv("HIGHLAND_API_KEY")


def _post_with_network_retry(url: str, **kwargs: Any) -> requests.Response:
    """Retry transient transport failures, including request-body write timeouts."""
    for attempt in range(LLM_MAX_RETRIES + 1):
        try:
            ...
            if attempt >= LLM_MAX_RETRIES:
                raise
            print(
                f"Network failure while calling model API; retrying "
                f"in {LLM_RETRY_DELAY_SECONDS}s ({attempt + 1}/{LLM_MAX_RETRIES})."
            )
            time.sleep(LLM_RETRY_DELAY_SECONDS)
    raise RuntimeError("Model API request failed without a response")
...
def _query_dashscope(args: "LaunchArgs | ModelQueryArgs", prompt: list[dict]) -> dict[str, Any]:
    """Call Qwen through the OpenAI-compatible Alibaba Cloud endpoint."""
    payload = {
        "model": args.model,
        "messages": prompt,
        "temperature": args.temperature,
        # CaP-X's global default (20K) is intended for reasoning APIs.
        # Cap Qwen output to keep robot-control turns responsive.
        "max_tokens": min(args.max_tokens, DASHSCOPE_MAX_TOKENS),
    }
    headers = {"Content-Type": "application/json"}
    api_key = _api_key_for_model(args.model, args.api_key)
    if api_key:
        headers["Authorization"] = f"Bearer {api_key}"
    server_url = (
        DASHSCOPE_SERVER_URL
        if args.server_url == HIGHLAND_SERVER_URL
        else args.server_url
    )
    start_time = time.monotonic()
    response = _post_with_network_retry(
        server_url, headers=headers, json=payload, timeout=LLM_REQUEST_TIMEOUT
    )
    ...
    try:
        ...
        raise RuntimeError(f"Unexpected DashScope response format: {body}") from exc
    if isinstance(content, list):
        content = "".join(item.get("text", "") for item in content if isinstance(item, dict))
    print(f"Time taken to query DashScope model: {time.monotonic() - start_time:.2f} seconds")
...
def query_model(args: "LaunchArgs | ModelQueryArgs", prompt: list[dict]) -> dict[str, Any]:
    ...
    if args.model == DASHSCOPE_MODEL:
        return _query_dashscope(args, prompt)  # type: ignore[return-value]
    ...
    retry = 0
    # A bounded retry prevents a dead provider from blocking the robot session.
    # 404 is a configuration/model error and should be reported immediately.
    while response.status_code in [500, 502, 503, 504] and retry < LLM_MAX_RETRIES:
        retry += 1
        print(
            f"Retry {retry}/{LLM_MAX_RETRIES}. Model query failed with status "
            f"{response.status_code}; retrying in {LLM_RETRY_DELAY_SECONDS} seconds."
        )
        time.sleep(LLM_RETRY_DELAY_SECONDS)
        response = _post_with_network_retry(
            server_url,
...
def query_model_streaming(
    ...
    """
    if args.model == DASHSCOPE_MODEL:
        # The compatible endpoint supports OpenAI SSE. Keep Qwen on the same
        # streaming path as Highland so the UI receives tokens immediately.
        payload = {
            "model": args.model,
            "messages": prompt,
            "temperature": args.temperature,
    ...
    with _post_with_network_retry(
        stream_server_url if args.model == DASHSCOPE_MODEL else args.server_url,
        headers=headers,
        data=json.dumps(payload),
        timeout=LLM_REQUEST_TIMEOUT,
        stream=True,
...
def query_model_ensemble(
    args: "LaunchArgs | ModelQueryArgs",
    prompt: list[dict],
    synthesis_model: str = HIGHLAND_MODEL,
    is_multiturn = False
) -> dict[str, Any]:
