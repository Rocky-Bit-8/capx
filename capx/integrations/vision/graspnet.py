...
def init_contact_graspnet(device: str = "cuda", checkpoint_path: str | None = None) -> Any:
    ...
    def plan(
        ...
        num_viewpoints = 10 # default to 10 viewpoints, and not passed as a parameter
        ...
        try:
            ...
            resp = requests.post(
                f"{SERVICE_URL}/plan", json=payload, timeout=REQUEST_TIMEOUT_SECONDS
            )
...
def init_contact_graspnet_point_clouds() -> Any:
    ...
    def plan_point_clouds(
        ...
        try:
            resp = requests.post(
                f"{SERVICE_URL}/plan_point_clouds", json=payload, timeout=REQUEST_TIMEOUT_SECONDS
            )
            resp.raise_for_status()
            data = resp.json()
        except requests.RequestException as e:
