...
def init_sam3(
    ...
    def segment_fn(
        ...
        # SAM3 can emit hundreds of overlapping proposals for broad prompts.
        # Returning all of them bloats code execution and WebSocket updates;
        # keep the strongest distinct candidates for policy-side selection.
        results.sort(key=lambda result: float(result.get("score", 0.0)), reverse=True)
        if len(results) > MAX_TEXT_PROMPT_RESULTS:
            print(
                f"SAM3 returned {len(results)} masks for '{text_prompt}'; "
                f"keeping top {MAX_TEXT_PROMPT_RESULTS}."
            )
            results = results[:MAX_TEXT_PROMPT_RESULTS]
        return results
...
def init_sam3_point_prompt(
    ...
    def point_prompt_fn(
        ...
        if (
            ...
            print(f"Skipping SAM3 point prompt with invalid coordinates: {point_coords!r}")
            return []
        encoded_image = _encode_image(image)
        payload = {
