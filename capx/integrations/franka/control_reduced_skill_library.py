...
class FrankaControlApiReducedSkillLibrary(FrankaControlApiReduced):
    ...
    def mask_to_world_points(
        ...
        # Thin/edge masks can land entirely on invalid simulator depth pixels.
        # Retry with a small expanded bounding box before returning no points.
        if not np.any(valid):
            mask_bool = np.asarray(mask) > 0
            ys_all, xs_all = np.where(mask_bool)
            depth_valid = np.isfinite(depth) & (depth > 0)
            if len(ys_all) and np.any(depth_valid):
                h, w = depth.shape
                pad = 8
                y0, y1 = max(0, int(ys_all.min()) - pad), min(h, int(ys_all.max()) + pad + 1)
                x0, x1 = max(0, int(xs_all.min()) - pad), min(w, int(xs_all.max()) + pad + 1)
                local_valid = depth_valid[y0:y1, x0:x1]
