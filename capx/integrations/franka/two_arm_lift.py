...
class FrankaTwoArmLiftApi(TwoArmExecutionTraceMixin, ApiBase):
    ...
    def __init__(self, env: BaseEnv) -> None:
        ...
        # Vision initialization.  OWL-ViT is intentionally optional here: the
        # robosuite lift environment already exposes handle observations, so a
        # missing detector must not prevent the task from starting.
    ...
    def _get_handle_pose_with_graspnet(
        ...
        # OWL-ViT is not required for this robosuite task.  Use the simulator's
        # object observation when available so the task can run offline without
        # downloading the OWL-ViT checkpoint.  This is a privileged fallback,
        # not a replacement for a real visual detector on hardware.
        pot_poses = obs.get("pot_poses")
        if isinstance(pot_poses, dict):
            pose_key = {
                "red box-like pot": "pot",
                "The green square frame": "handle0",
                "The blue square": "handle1",
            }.get(object_name)
            if pose_key in pot_poses:
                bbox_center = np.asarray(pot_poses[pose_key][:3], dtype=np.float64)
                print(f"OWL-ViT skipped; using simulator pose for {object_name}.")
        ...
        if self.owl_vit_det_fn is None:
            raise RuntimeError(
                f"OWL-ViT is disabled and simulator pose is unavailable for {object_name}."
            )
