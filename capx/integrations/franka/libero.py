    ...
    def plan_grasp_from_point_clouds(
        ...
        if len(grasp_sample) == 0:
            ...
            if fallback is not None:
                print("Contact-GraspNet returned no point-cloud grasps; using centroid fallback")
                grasp_sample = fallback[None, ...]
                grasp_scores = np.array([0.0], dtype=np.float64)
            else:
                print("Contact-GraspNet returned no grasps and segmented point cloud is empty")
