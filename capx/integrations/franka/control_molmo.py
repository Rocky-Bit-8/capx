...
from capx.integrations.motion.pyroki import init_pyroki_local, init_pyroki_remote
...
from capx.integrations.franka.cube_safety import FrankaCubeSafetyApi
...
# ------------------------------- Control API ------------------------------
class FrankaControlMolmoApi(FrankaCubeSafetyApi):
    ...
    Functions:
      ...
      - open_gripper() -> None
      - close_gripper() -> None
      - point_prompt_molmo(text_prompt: str) -> dict[str, tuple[int | None, int | None]]
    """
    ...
    def __init__(
        ...
        if not real and hasattr(env, "robosuite_env"):
            self._TCP_OFFSET = np.zeros(3, dtype=np.float64)
    ...
    def point_prompt_molmo(
        ...
        """Ask the remote Molmo service to point to an object in the current RGB view."""
        ...
        if not rgb_images:
            raise RuntimeError("No RGB images in observation")

        image = next(iter(rgb_images.values()))
        self._log_step(
            "Molmo Point Prompt",
            f"Querying Molmo for '{text_prompt}' ...",
            images=image,
        )
        result = self.molmo_point_fn(Image.fromarray(image).convert("RGB"), objects=[text_prompt])
        self._log_step_update(text=f"Molmo result: {result}")
        return result

    # Keep the cube-stack clearance maneuver from FrankaCubeSafetyApi.  Its
    # implementation delegates the final IK motion to the original control
    # API, so the Molmo point service remains orthogonal to robot motion.
    def goto_pose(
        self, position: np.ndarray, quaternion_wxyz: np.ndarray, z_approach: float = 0.0
    ) -> None:
        FrankaCubeSafetyApi.goto_pose(self, position, quaternion_wxyz, z_approach)

    def home_pose(self) -> None:
        FrankaCubeSafetyApi.home_pose(self)
    ...
    def sample_grasp_pose(self, object_name: str) -> tuple[np.ndarray, np.ndarray]:
        ...
        start_time = time.time()
        if not self.real and hasattr(self._env, "robosuite_env") and "cube" in object_name:
            pos, _, _ = self.get_object_pose(object_name, return_bbox_extent=False)
            quat = np.array([0.0, 0.0, 1.0, 0.0], dtype=np.float64)
            print(f"Grasp sample position for {object_name}: {pos}")
            print(f"Grasp sample quaternion wxyz for {object_name}: {quat}")
            return pos, quat
        ...
        grasp_pos = grasp_sample_tf_world.wxyz_xyz[-3:].copy()
        if not self.real and hasattr(self._env, "robosuite_env"):
            grasp_pos = grasp_pos + np.array([0.0, 0.0, -0.03], dtype=np.float64)

        elapsed = time.time() - start_time
        n_candidates = len(self._env.grasp_scores)
        best_score = float(self._env.grasp_scores.max())
        pos_str = np.array2string(grasp_pos, precision=4)
        self._log_step_update(
        ...
        return grasp_pos, grasp_sample_tf_world.wxyz_xyz[:4]
