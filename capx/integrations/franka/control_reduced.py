...
from capx.integrations.motion.pyroki import init_pyroki_local, init_pyroki_trajopt_local
...
# ------------------------------- Control API ------------------------------
class FrankaControlApiReduced(TwoArmExecutionTraceMixin, ApiBase):
    """
    ...
    def __init__(
        ...
        # Pyroki now solves IK in-process against the active MuJoCo scene.
        # The previous server-shaped call omitted env and made every reduced
        # API fail during construction.
        self.ik_solve_fn = init_pyroki_local(self._env, arm_index=0)
        self.ik_solve_fn_arm1 = (
            init_pyroki_local(self._env, arm_index=1) if self.bimanual else None
        )
        self.trajopt_plan_fn = init_pyroki_trajopt_local(self._env)
        self.cfg = None
        self.cfg_arm1 = None
    ...
    def functions(self) -> dict[str, Any]:
        ...
        # Molmo runs as an optional, separately started service. Do not let
        # generated code depend on it until the operator explicitly enables it.
        if os.environ.get("CAPX_ENABLE_MOLMO", "").lower() in {"1", "true", "yes"}:
        ...
        # Bimanual reduced APIs are used by the two-arm lift/handover config
        # families. Keep single-arm reduced APIs unchanged.
        return self._trace_functions(fns) if self.bimanual else fns
    ...
    def segment_sam3_point_prompt(
        self,
        rgb: np.ndarray,
        point_coords: tuple[float | None, float | None],
    ) -> list[dict[str, Any]]:
        ...
        """
        if (
            not isinstance(point_coords, (tuple, list))
            or len(point_coords) != 2
            or any(coord is None or not np.isfinite(coord) for coord in point_coords)
        ):
            self._log_step("SAM3 Point Segmentation", "Skipped: point prompt is missing or invalid.", images=rgb)
            self._log_step_update(text="No valid point coordinate was supplied.")
            return []

        self._log_step(
            "SAM3 Point Segmentation",
            f"Running SAM3 point-prompt at ({point_coords[0]}, {point_coords[1]}) …",
            images=rgb,
        )
    ...
    def solve_ik_arm1(self, position: np.ndarray, quaternion_wxyz: np.ndarray) -> np.ndarray:
        ...
        if self.ik_solve_fn_arm1 is None:
            raise RuntimeError("Arm 1 IK is unavailable for a single-arm environment")
        self.cfg_arm1 = solve_ik_with_convergence(
            self.ik_solve_fn_arm1, quat_wxyz_arm1, offset_pos, self.cfg_arm1
        )
        return extract_arm_joints(self.cfg_arm1)
