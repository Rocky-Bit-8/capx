...
from capx.integrations.franka.two_arm_trace import TwoArmExecutionTraceMixin

# ------------------------------- Hammer Handover API ------------------------------
class FrankaHandoverApi(TwoArmExecutionTraceMixin, ApiBase):
    ...
    def __init__(self, env: BaseEnv) -> None:
        ...
        self.ik_solve_fn = init_pyroki_local(self._env, arm_index=0)
        self.ik_solve_fn_arm1 = init_pyroki_local(self._env, arm_index=1)
    ...
    @staticmethod
    def _select_depth_supported_mask(
        results: list[dict[str, Any]], depth: np.ndarray
    ) -> np.ndarray:
        """Choose the strongest SAM3 proposal that overlaps metric depth.

        The handover camera is deliberately mounted high above the table, so
        object depth is commonly 2--4 m.  Selection must therefore validate
        only finite positive depth, rather than applying a tabletop-specific
        upper bound such as 2 m.
        """
        depth_2d = np.asarray(depth).squeeze()
        if depth_2d.ndim != 2:
            raise ValueError(f"Expected a 2D depth image, got {depth_2d.shape}.")
        valid_depth = np.isfinite(depth_2d) & (depth_2d > 0)

        candidates: list[tuple[float, np.ndarray]] = []
        for result in results:
            mask = np.asarray(result.get("mask"), dtype=bool)
            if mask.shape != depth_2d.shape:
                try:
                    mask = mask.reshape(depth_2d.shape)
                except ValueError:
                    continue
            count = int(np.count_nonzero(mask & valid_depth))
            if count:
                candidates.append((float(result.get("score", 0.0)), mask))

        if not candidates:
            raise RuntimeError(
                "SAM3 proposals do not overlap any valid depth pixels for the requested object."
            )

        # Preserve SAM3's original score ordering after excluding proposals
        # that cannot be converted into any 3D point.
        _, mask = max(candidates, key=lambda candidate: candidate[0])
        return mask

    @staticmethod
    def _masked_camera_points(
        mask: np.ndarray, depth: np.ndarray, intrinsics: np.ndarray
    ) -> np.ndarray:
        """Unproject mask pixels without changing their pixel indexing."""
        depth_2d = np.asarray(depth).squeeze()
        mask = np.asarray(mask, dtype=bool)
        valid = mask & np.isfinite(depth_2d) & (depth_2d > 0)
        ys, xs = np.where(valid)
        if len(ys) == 0:
            raise RuntimeError("Selected SAM3 mask contains no valid metric depth pixels.")
    ...
    def get_object_pose(self, object_name: str, return_bbox_extent: bool = False) -> tuple[np.ndarray, np.ndarray, np.ndarray | None]:
        ...
        mask = self._select_depth_supported_mask(sam3_results, depth)
        ...
        points = self._masked_camera_points(mask, depth, obs[camera_name]["intrinsics"])
    ...
    def goto_pose_arm1(
        ...
        if (z_approach != 0.0):  # If z_approach is not 0.0, approach the object from above by z_approach meters
            ...
            self.cfg_1 = self.ik_solve_fn_arm1(
                target_pose_wxyz_xyz=np.concatenate([quat_wxyz, z_offset_pos]),
                prev_cfg=self.cfg_1,
            )
    ...
    def sample_grasp_pose(self, object_name: str, arm_name: str) -> tuple[np.ndarray, np.ndarray]:
        ...
        valid_mask = mask & np.isfinite(np.asarray(depth).squeeze()) & (np.asarray(depth).squeeze() > 0)
        self._env.cube_points = self._masked_camera_points(mask, depth, obs[camera_name]["intrinsics"])
        self._env.cube_color = np.asarray(rgb)[valid_mask] / 255.0
