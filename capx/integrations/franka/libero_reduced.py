...
from capx.integrations.franka.common import (
    ...
    make_top_down_grasp_from_points as _make_top_down_grasp,
...
class FrankaLiberoApiReduced(ApiBase):
        ...
        if len(grasp_sample) == 0:
            if segmentation.any():
                ...
                if np.any(valid):
                    ...
                    points = np.stack(
                        [
                            (xx[valid] - cx) * z[valid] / fx,
                            (yy[valid] - cy) * z[valid] / fy,
                            z[valid],
                        ],
                        axis=1,
                    )
                    fallback = _make_top_down_grasp(points)
                    if fallback is not None:
                        logger.warning("Contact-GraspNet returned no grasps; using depth-mask fallback")
                        grasp_sample = fallback[None, ...]
                        grasp_scores = np.array([0.0], dtype=np.float64)
            if len(grasp_sample) == 0:
                logger.warning("Contact-GraspNet returned no grasps and no valid depth-mask fallback exists")
    ...
    def plan_grasp_from_point_clouds(
        ...
        if len(grasp_sample) == 0:
            fallback = _make_top_down_grasp(pc_segment)
            if fallback is not None:
                logger.warning("Contact-GraspNet returned no point-cloud grasps; using centroid fallback")
                grasp_sample = fallback[None, ...]
                grasp_scores = np.array([0.0], dtype=np.float64)
            else:
                logger.warning("Contact-GraspNet returned no grasps and segmented point cloud is empty")
                return np.empty((0, 4, 4), dtype=np.float64), np.empty((0,), dtype=np.float64)
