...
def get_oriented_bounding_box_from_3d_points(points: np.ndarray) -> dict[str, Any]:
    ...
    if points.ndim != 2 or points.shape[1] != 3 or len(points) == 0:
        raise ValueError(
            "Cannot compute an oriented bounding box: no valid 3D points were produced "
            "by the mask/depth intersection."
        )
    points = points[np.all(np.isfinite(points), axis=1)]
    if len(points) == 0:
        raise ValueError("Cannot compute an oriented bounding box from non-finite 3D points.")
...
def make_top_down_grasp_from_points(points: np.ndarray) -> np.ndarray | None:
    """Build a conservative top-down grasp pose at the center of a point set.

    The pose uses a fixed gripper orientation whose local z axis points down.
    It is intended as a recovery candidate when Contact-GraspNet returns no
    grasps, not as a replacement for learned grasp ranking.
    """
    points = np.asarray(points, dtype=np.float64)
    ...
    pose[:3, 3] = np.median(points, axis=0)
    return pose
