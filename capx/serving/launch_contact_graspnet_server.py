...
def main(device: str = "cuda", port: int = 8115, host: str = "127.0.0.1"):
    ...
    if device.startswith("cuda") and torch.cuda.is_available():
        try:
            ...
            if free_bytes < 2 * 1024**3:
                logger.warning(
                    "Only %.2f GiB CUDA memory free; falling back to CPU Contact-GraspNet.",
                    free_bytes / 1024**3,
                )
                _DEVICE = device = "cpu"
        except Exception as e:
            logger.warning(f"Could not query CUDA memory; keeping device={device}: {e}")
    ...
    # Checkpoints are used for inference only.  Keeping the default training
    # mode enables dropout and batch-statistics updates, which can destroy the
    # grasp confidence scores and leave no valid candidates after filtering.
    _GRASP_ESTIMATOR.model.eval()
    logger.info("Contact-GraspNet model set to eval mode")
