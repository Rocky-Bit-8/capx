...
def _load_robot_urdf(robot_urdf_or_name: str):
    import yourdfpy  # type: ignore

    if os.path.exists(robot_urdf_or_name):
        urdf_path = os.path.abspath(robot_urdf_or_name)
        return yourdfpy.URDF.load(urdf_path, mesh_dir=os.path.dirname(urdf_path))

    if robot_urdf_or_name == "panda_description":
        base_path = (
        ...
        urdf_path = base_path / "urdf" / "panda_arm_hand.urdf"
        if urdf_path.exists():
            logger.info(f"Using checked-in Panda URDF: {urdf_path}")
            return yourdfpy.URDF.load(str(urdf_path), mesh_dir=str(base_path))
        logger.warning(f"Checked-in Panda URDF not found at {urdf_path}; falling back to robot_descriptions")

    from robot_descriptions.loaders.yourdfpy import load_robot_description

    return load_robot_description(robot_urdf_or_name)
...
def init_pyroki_server(
    ...
    urdf = set_min_distance_from_limits(_load_robot_urdf(robot_urdf_name))
...
def _do_solve_ik(target_pose_wxyz_xyz: np.ndarray, prev_cfg: np.ndarray | None) -> list[float]:
    ...
    # The checked-in Panda URDF (`panda_arm_hand.urdf`) has 8 actuated DOFs
    # (7 arm joints + hand finger joints). CAP-X only drives the 7 arm joints,
    # so drop any trailing hand/finger joints from the IK solution.
    return list(map(float, q[:7]))
